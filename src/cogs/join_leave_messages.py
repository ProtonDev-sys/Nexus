...
class WelcomeGoodbyeCog(commands.Cog):
    ...
    async def set_welcome_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        self.data_manager.set_guild_setting(interaction.guild_id, 'welcome_channel', channel.name)
    ...
    async def set_leave_channel(self, interaction: discord.Interaction, channel: discord.TextChannel):
        self.data_manager.set_guild_setting(interaction.guild_id, 'leave_channel', channel.name)
        await interaction.response.send_message(f"Leave channel set to: {channel.mention}")

    @commands.Cog.listener()
    async def on_member_join(self, member):
        welcome_channel_name = self.data_manager.get_guild_setting(member.guild.id, 'welcome_channel')
    ...
    async def on_member_remove(self, member):
        leave_channel_name = self.data_manager.get_guild_setting(member.guild.id, 'leave_channel')
        if leave_channel_name:
            channel = discord.utils.get(member.guild.channels, name=leave_channel_name)
            if channel:
